...
public class ShooterAngleError {
	...
	public static double calculateError(final double distToTower) {
		...
		final double b = saeCosRule(a, c, Brad);

		// We want to calculate the angle error at C
		final double Crad = saeSinRuleTwoLen(b, c, Brad);

		// Return the number of degrees
		return Math.toDegrees(Crad);
	}

	public static double saeCosRule(final double a, final double c, final double Brad) {
		// a² = b² + c² - 2bc cosA
		return Math.sqrt((a * a) + (c * c) - (2 * a * c * Math.cos(Brad)));
	}

	// will return Crad
	public static double saeSinRuleTwoLen(final double b, final double c, final double Brad) {
		// a/sin(A) = b/sin(B)
		// sin B = (b sin(A)) / a
		return Math.asin((c * Math.sin(Brad)) / b);
	}

	// will return b
	public static double saeSinRuleTwoAng(final double Arad, final double a, final double Brad) {
		// a/sin(A) = b/sin(B)
		// b = ( a * sin(B) ) /sin(A)
		return (a * Math.sin(Brad)) / Math.sin(Arad);
	}

	public static void main(String args[]) {
		System.out.println("For testing");
		// System.out.println("When 5 feet away b dist is: " +
		// saeCosRule(5*12));
		// System.out.println("When 8 feet away b dist is: " +
		// saeCosRule(8*12));
		// System.out.println("When 15 feet away b dist is: " +
		// saeCosRule(15*12));
